import os, coloredlogs, logging, random, numpy as np, io
from bs4 import BeautifulSoup

VAL_RATIO = 0.1
SOURCE_FOLDER = '/home/urban-watch/plate_detect/dataset'#'E:/Projects/ocr_datasets/alpr/detect2'
DEST_FOLDER = '/home/urban-watch/plate_detect/dataset'#E:/Projects/Garbage/plate_detect_dataset'
...
# Dataset built as explained at https://github.com/yhenon/pytorch-retinanet
...
if __name__ == '__main__':
    ...
    val_frac = int(100 * VAL_RATIO)
    csv_train = open(os.path.join(DEST_FOLDER, 'train.csv'), 'w')
    csv_val = open(os.path.join(DEST_FOLDER, 'val.csv'), 'w')
    ...
    for i, folder in enumerate(FOLDERS):
        ...
        for j, file in enumerate(files):                
            ...
            if ',' in file: # must not contain ","
                logger.error('{} has special char'.format(file))
                continue
            ...
            csv_file = csv_val if (random.choice(val_pop) == 0) else csv_train
            ...
            csv_file.write(
                '{},,,,,\n'.format(file) if len(entries)==0 else entries
            )
    ...
    # Write classes
    with open(os.path.join(DEST_FOLDER, 'classes.csv'), 'w') as f:
