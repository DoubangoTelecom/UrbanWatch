...
class NpuFriendlyHardSigmoid(nn.Module):
    def __init__(self, inplace: bool = False):
        super(NpuFriendlyHardSigmoid, self).__init__()
        self.inplace = inplace
    def forward(self, x):
        assert(len(x.shape) == 4)
        const_3 = torch.tensor([[[[3.0]]]], dtype=torch.float32).to(x.device)
        const_6 = torch.tensor([[[[1.0/6.0]]]], dtype=torch.float32).to(x.device)
        out = F.relu6(x + const_3, inplace=self.inplace) * const_6
        return out
...
class SqueezeExcite(nn.Module):
    def __init__(
        ...
        activation="ReLU",
        gate_fn=NpuFriendlyHardSigmoid(),
        divisor=4,
        adapt_pooling=True,
        **_
    ):
        ...
        self.adapt_avgpooling = nn.AdaptiveAvgPool2d(1) if adapt_pooling else None
    ...
    def forward(self, x):
        if self.adapt_avgpooling:
            x_se = self.adapt_avgpooling(x)
        else:
            x_se = F.avg_pool2d(x, x.size()[2:])
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(
        ...
        if has_se:
            self.se = SqueezeExcite(mid_chs, se_ratio=se_ratio, adapt_pooling=adapt_pooling)
        else:
            self.se = None
