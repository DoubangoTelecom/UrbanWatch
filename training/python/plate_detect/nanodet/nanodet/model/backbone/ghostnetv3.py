    ...
    def forward(self, x):
        if self.mode in ['ori']:
            ...
            if self.gate_loc=='before':
                return out[:,:self.oup,:,:]*F.interpolate(self.gate_fn(res/self.scale),size=out.shape[-2:],mode=self.inter_mode) # 'bilinear'
#                 return out*F.interpolate(self.gate_fn(res/self.scale),size=out.shape[-1].item(),mode=self.inter_mode) # 'bilinear'
            else:
...
class GhostNet(nn.Module):
    def __init__(self, cfgs, out_stages=(4, 6, 9), width=1.0, block=GhostBottleneck, args=None):
...
def ghostnetv3(**kwargs):
    ...
    return GhostNet(cfgs, width=kwargs['width'])
