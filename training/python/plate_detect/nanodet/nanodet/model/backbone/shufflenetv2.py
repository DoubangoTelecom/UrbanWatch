...
class ChannelShuffle(nn.Module):
    def __init__(self, groups, output_channels):
        super(ChannelShuffle, self).__init__()
        self.groups = groups
        import numpy as np
        self.output_channels = output_channels
        self.group_len = output_channels // groups
        self.indexes = torch.arange(output_channels).reshape(self.groups, self.group_len).transpose(1, 0).flatten().int()
        
    def forward(self, x):
        if False: # NPU-friendly (not faster)
            #   -> https://gitee.com/ascend/pytorch/blob/67c590693e88aa920a2228494a9e833713aadb48/docs/en/PyTorch%20Network%20Model%20Porting%20and%20Training%20Guide/PyTorch%20Network%20Model%20Porting%20and%20Training%20Guide.md#python-optimization-details
            return x.index_select(1, self.indexes)
        else:
...
class ShuffleV2Block(nn.Module):
    def __init__(self, inp, oup, stride, activation="ReLU"):
        ...
        self.shuffle = ChannelShuffle(2, oup)
...
class ShuffleNetV2(nn.Module):
    def __init__(
        ...
        if model_size == "0.25x":
            self._stage_out_channels = [16, 24, 48, 128, 512]
