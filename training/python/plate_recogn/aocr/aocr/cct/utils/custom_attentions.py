...
class MultiheadGQA(nn.Module):
    ...
    def __init__(
        ...
        elif (hidden_size % self.query_heads != 0) or (hidden_size % self.kv_heads != 0):
            raise ValueError(
        ...
        if not head_dim % 8 == 0:
            raise ValueError(
                f"head_dim (hidden_size / num_heads = {head_dim}) must be divisible by 8"
            )
        if not head_dim <= 128:
            raise ValueError(
                f"head_dim (hidden_size / num_heads = {head_dim}) must be <= 128"
            )

        # Hidden layer
        self.h_proj = nn.Linear(
            embed_dim, hidden_size, bias=bias
        )
        ...
        kv_embed_dim = hidden_size // query_heads * kv_heads
        self.k_proj = nn.Linear(
            hidden_size, kv_embed_dim, bias=bias
        )
        self.v_proj = nn.Linear(
            hidden_size, kv_embed_dim, bias=bias
        )
        self.norm: Optional[DynamicTanh] = None
        if layer_norm:
            self.norm = DynamicTanh(hidden_size)
        ...
        self.out_proj = nn.Linear(
            hidden_size, out_features, bias=bias
        )

        self._reset_parameters()

    def _reset_parameters(self):
        if not self.npu_friendly:
