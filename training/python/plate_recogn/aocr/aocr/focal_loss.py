...
class FocalLossCTC(nn.Module):
    ...
    def forward(self, log_probs: torch.Tensor, targets: torch.Tensor, input_lengths: torch.Tensor, target_lengths: torch.Tensor) -> torch.Tensor:
        ...
        focal_ctc_loss = torch.multiply(torch.multiply(self.alpha, torch.pow((1-p), self.gamma)), ctc_loss) #((alpha)*((1-p)**gamma)*(ctc_loss))
