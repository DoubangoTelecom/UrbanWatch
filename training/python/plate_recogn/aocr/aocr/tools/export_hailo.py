...
def parse_args():
    ...
    parser.add_argument("--target", type=str, required=True, help="Must be 'anpr_latin' or 'anpr_chinese' or 'anpr_korean'")
    parser.add_argument(
...
def main(cfg, args):
    assert args.target in ['anpr_latin', 'anpr_chinese', 'anpr_korean'], '--target value must be "anpr_latin" or "anpr_chinese" or "anpr_korean"'
    model = AOCR(cfg, training=False).eval()
    model.load_state_dict(torch.load(args.model_path, map_location=lambda storage, loc: storage, weights_only=True))
    ...
    model_script = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../scripts/', f'hailo_aocr_{args.target}.alls')
    ...
    for i, platform in enumerate(platforms):
        ...
        model_folder = os.path.join(hailo_folder, f'aocr_{args.target}')
        ...
        shutil.move(os.path.join(model_folder, 'temp.hef'), os.path.join(hailo_folder, f'aocr_{args.target}.{platform}'))
...
if __name__ == "__main__":
    ...
    args = parse_args()        
    cfg = Config.parse(args.cfg_path)
    main(cfg, args)
    print('!!!Done!!!')
