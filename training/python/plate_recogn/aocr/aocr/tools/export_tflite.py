...
def tflite_quant_representative_dataset_dummy(ch, h, w):
    for _ in range(10):
        image = np.random.rand(1, ch, h, w).astype(np.float32)
        yield [image]
...
def export(cfg, opt):
    ...
    converter.representative_dataset = lambda: tflite_quant_representative_dataset_dummy(*sample_args[0].shape[1:])
